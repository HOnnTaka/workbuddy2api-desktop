...
/* ── 路由 ─────────────────────────────────────────────────────────── */
const TITLES = { about: '关于与版本', accounts: '账号池', usage: '用量', packages: '积分构成', taskscenter: '任务中心', models: '模型与档位', config: '配置', logs: '运行日志' };
function go(v) {
  view = v;
...
(function() {
  ...
  async function checkServerAlive() {
    try {
      // 优先探测静态页面或 healthz 接口
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 1200);
      const resp = await fetch('/healthz', { cache: 'no-cache', signal: controller.signal });
      clearTimeout(timeoutId);

      // 无论 200 还是 503（空池），有 HTTP 响应即表示 Go 内核已在监听并可服务
      if (resp.status >= 200 && resp.status < 600) {
        if (offlineCount > 0 || isRestarting) {
          offlineCount = 0;
          isRestarting = false;
          if (shield) shield.style.display = 'none';
          toast('核心服务已就绪并恢复连接', 'ok');
    ...
    offlineCount++;
    if (offlineCount >= 2 || isRestarting) {
      if (shield) shield.style.display = 'flex';
      if (countEl) countEl.textContent = '已探测 ' + offlineCount + ' 次';
    }
    return false;
  }

  // 1.5 秒周期探活
  setInterval(checkServerAlive, 1500);

  // 供 Tauri 托盘或内部调用的全局钩子
  window.__wb2api_trigger_restart_ui = function() {
    isRestarting = true;
    offlineCount = 1;
    if (shield) shield.style.display = 'flex';
    if (countEl) countEl.textContent = '正在重启核心进程...';
  ...
  window.__wb2api_retry_reconnect = function() {
    if (countEl) countEl.textContent = '正在重新建立连接...';
    checkServerAlive().then(alive => {
      if (!alive) {
        toast('服务暂未就绪，继续等待...', 'warn');
      }
    });
  };
